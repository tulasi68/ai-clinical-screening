...
/** Digits-only Indian phone with country code (e.g. 919844771724). */
...
export async function sendInitial(to, patientName, complaint) {
  ...
  if (name) {
    return sendTemplate(
      to,
      name,
      process.env.WA_INITIAL_TEMPLATE_LANGUAGE || 'en',
      [{
        type: 'body',
        parameters: [
          { type: 'text', text: String(patientName || 'Patient') },
          { type: 'text', text: String(complaint || 'your health concern') }
        ]
      }]
    );
  }
  return sendText(
    to,
    `Hello ${patientName || 'there'}. We received your information about ${complaint || 'your health concern'}. I will ask a few questions to collect information for your healthcare professional. This is not a diagnosis or prescription. Please answer in your own words.`
  );
}
